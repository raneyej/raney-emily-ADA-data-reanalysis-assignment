...
Trauma_comp_5 <- data.frame(Region = c("Head.Neck", "Trunk", "Shoulder.Arm", "Hand", "Pelvis", "Leg", "Foot"),
                            Neandertals_no_DJD = c(0.304,	0.174,	0.304,	0.043,	0,	0.13,	0.043), 
                            London_value = c(0.062,	0.07,	0.316,	0.244,	0.002,	0.236,	0.07),
                            New_York_value = c(0.137,	0.123,	0.253,	0.219,	0.005,	0.206,	0.056), 
                            New_Mexico_value = c(0.016,	0.125,	0.231,	0.236,	0.021,	0.111,	0.259))
Trauma_comp_5_long <- gather(Trauma_comp_5, Sample, Proportion, -Region)
Trauma_comp_5_long
Trauma_comp_5_long$Region <- factor(Trauma_comp_5_long$Region, levels = c("Head.Neck", "Trunk", "Shoulder.Arm", "Hand", "Pelvis", "Leg", "Foot"))
Trauma_bar_5 <- ggplot(data = Trauma_comp_5_long, aes(x = Region, y = Proportion, fill = Sample)) +
  geom_col(position = position_dodge())
Trauma_bar_5 + scale_fill_manual(values = c("turquoise3", "mediumblue", "darkorange", "purple1"))+labs(caption  = "Figure 6: Traumatic injury distribution comparison between Neandertals without DJD and cinical extant human samples")
```
...
Trauma_bar_4 <- ggplot(data = Trauma_comp_4_long, aes(x = Region, y = Proportion, fill = Sample)) +
  geom_col(position = position_dodge()) 
Trauma_bar_4+scale_fill_manual(values = c("hotpink", "purple2"))+labs(caption = "Figure 7: Traumatic injury distribution comparison of Neandertals and rodeo performers")
```
...
Trauma_bar_3 <- ggplot(data = Trauma_comp_3_long, aes(x = Region, y = Proportion, fill = Sample)) +
  geom_col(position = position_dodge()) 
Trauma_bar_3+scale_fill_manual(values = c("hotpink", "springgreen2", "royalblue1", "magenta3", "mediumblue", "darkorange", "purple1",  "red2"))+labs(caption = "Figure 8: Traumatic injury distribution comparison of Neandertals and all extant human samples")
```
